...
typedef struct MaterialMap {
    Texture2D texture;      // Material mapLimits texture
    Color color;            // Material mapLimits color
    float value;            // Material mapLimits value
} MaterialMap;
...
typedef enum {
    ...
    CUBEMAP_LAYOUT_PANORAMA                 // Layout is defined by a panorama image (equirrectangular mapLimits)
} CubemapLayout;
...
RLAPI Mesh GenMeshCubicmap(Image cubicmap, Vector3 cubeSize);                               // Generate cubes-based mapLimits mesh from image data
...
RLAPI void SetMaterialTexture(Material *material, int mapType, Texture2D texture);          // Set texture for a material mapLimits type (MATERIAL_MAP_DIFFUSE, MATERIAL_MAP_SPECULAR...)
